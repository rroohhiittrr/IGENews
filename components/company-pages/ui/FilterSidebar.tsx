...
export function FilterSidebar({
  ...
  const SectionHeader = ({ title, isOpen, onToggle }: { title: string; isOpen: boolean; onToggle: () => void }) => (
    <button
      onClick={onToggle}
      className="flex justify-between items-center w-full text-xs font-bold text-[#1E3A5F] dark:text-white uppercase tracking-wider mb-3 cursor-pointer"
    >
            ...
            {meta.tiers.map(t => (
              <label key={t.value} className="flex items-center gap-3 text-xs font-semibold text-gray-600 dark:text-gray-300 cursor-pointer">
                <input
                  ...
                  className="rounded border-gray-300 text-[#1E3A5F] focus:ring-[#1E3A5F] w-4 h-4 cursor-pointer"
                />
                <span>{t.label}</span>
