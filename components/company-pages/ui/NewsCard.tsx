    ...
    >
      ...
      <div className="relative aspect-[1.6] w-full rounded-2xl overflow-hidden bg-gray-100 dark:bg-white/5 mb-4 shadow-inner">
        {news.thumbnailUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#1E3A5F] to-[#2F6FA3] text-white/20">
        ...
        {/* Sponsored / PR tag */}
        {news.isSponsored && (
          <span className="absolute top-3 left-3 bg-[#F4A024] text-white text-[9px] font-black tracking-wider uppercase px-2.5 py-1 rounded-md shadow-md">
            Sponsored PR
          </span>
        )}
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-wider">
          <span className="text-[#1E3A5F] dark:text-[#F4A024]">{news.company.name}</span>
          <span>•</span>
          <span>{formattedDate}</span>
        </div>

        <h3 className="font-extrabold text-sm text-[#1E3A5F] dark:text-white leading-snug group-hover:text-[#F4A024] transition-colors line-clamp-2 h-10">
          {news.title}
        </h3>
