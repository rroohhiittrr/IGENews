  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-12">
      <div className="bg-white dark:bg-[#122238] border border-gray-100 dark:border-white/5 rounded-[32px] p-6 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col lg:flex-row items-center justify-between gap-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#F0652E]/5 rounded-full blur-3xl pointer-events-none" />
        
        <div className="space-y-3 max-w-xl text-center lg:text-left relative z-10">
          <span className="text-[10px] font-black uppercase tracking-wider text-[#F0652E] flex items-center justify-center lg:justify-start gap-1">
            <Sparkles className="w-3.5 h-3.5 fill-[#F0652E] stroke-none animate-pulse" />
            Stay Ahead
          </span>
          <h3 className="text-2xl font-black text-[#1D1D46] dark:text-white" style={{ fontFamily: 'var(--font-display)' }}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex items-center w-full">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5 pl-11 pr-4 py-3 rounded-2xl text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-[#F0652E] focus:bg-white"
                />
              </div>
              <button
                type="submit"
                className="bg-[#1D1D46] hover:bg-[#0642BA] text-white px-6 py-3 rounded-2xl font-bold text-xs shrink-0 cursor-pointer shadow-md transition-colors"
              >
                Subscribe
              </button>
            </form>
