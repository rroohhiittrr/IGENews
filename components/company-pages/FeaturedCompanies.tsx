    ...
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-12 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="space-y-1">
          <span className="text-[10px] font-black uppercase tracking-wider text-[#F0652E] flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 fill-[#F0652E] stroke-none" />
      ...
      {loading ? (
        ...
        <div className="text-center py-10 bg-white dark:bg-[#122238] border border-red-100 rounded-3xl p-6">
          ...
          <button onClick={() => setActiveTab(activeTab)} className="mt-2 text-xs font-bold text-[#1D1D46] dark:text-white underline">
            Retry Loading
          </button>
        </div>
      ) : list.length === 0 ? (
      ...
      {/* Upgrade CTA strip */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-gradient-to-r from-[#1D1D46]/5 to-[#0642BA]/5 dark:from-white/5 dark:to-transparent rounded-3xl p-5 border border-gray-100 dark:border-white/5">
        ...
        <Link
          ...
          className="flex items-center gap-1.5 text-xs font-black text-[#F0652E] hover:text-[#1D1D46] dark:hover:text-white transition-colors"
        >
          View Business Plans
