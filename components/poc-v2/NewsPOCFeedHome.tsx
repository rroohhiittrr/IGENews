...
export default function NewsPOCFeedHome() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("ign_v2_ai_plus");
    setAiPlusActive(stored === "true");

    const handleGlobalChange = () => {
      setAiPlusActive(localStorage.getItem("ign_v2_ai_plus") === "true");
    };

    window.addEventListener("ign_v2_ai_plus_changed", handleGlobalChange);
    return () => {
      window.removeEventListener("ign_v2_ai_plus_changed", handleGlobalChange);
    };
  }, []);
            ...
            .map((item) => (
              ...
              >
                ...
                <div className="flex-1 flex flex-col justify-between space-y-4">
                  <Link href={`/en/poc-v2/article/${item.id}`} className="space-y-2 block group">
              ...
              <Link href={`/en/poc-v2/article/${item.id}`} className="w-full md:w-80 h-48 md:h-full min-h-[160px] rounded-xl overflow-hidden relative block">
                <img 
                  src={item.image} 
                  alt={item.title}
      ...
      <section className="mx-auto max-w-7xl px-4 pt-12 lg:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-gray-200 dark:border-gray-800 pb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link 
              href="/poc-v2/feed/sector/intelligence" 
