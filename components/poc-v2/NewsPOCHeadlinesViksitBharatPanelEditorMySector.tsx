...
const MOCK_SCHEMES: GovernmentScheme[] = [
  { id: "sch-1", name: "Sovereign AI Compute Infrastructure Subsidy", ministry: "MeitY", sectorName: "AI & Cyber Security", eligibility: "Startups using sovereign hosting", benefit: "Up to 35% reimbursement on GPU hosting costs", deadline: "Oct 15, 2026" },
  { id: "sch-2", name: "Advanced Cell Chemistry Manufacturing Incentive", ministry: "MHI", sectorName: "Automotive & Electric Vehicles", eligibility: "Sodium grid battery builders", benefit: "9% cash-back on sodium raw sourcing CapEx", deadline: "Nov 30, 2026" }
];
...
export default function NewsPOCHeadlinesViksitBharatPanelEditorMySector({ onBack }: Props) {
  ...
  }, [selectedSectorFilter, selectedPolicyType]);
  ...
  const handleToggleTrackPolicy = (id: string) => {
    if (trackedPolicies.includes(id)) {
      ...
      showToast("Policy added to tracking dashboard ✓");
    }
  };
  ...
  const handleAskAi = (q: string) => {
    ...
    setTimeout(() => {
      ...
      if (q.toLowerCase().includes("risk") || q.toLowerCase().includes("compliance")) {
        setAiChatResponse("AI Policy Impact Assessment: Compliance changes in sovereign database hosting carry up to 4% capital penalties. Sourcing teams must audit database relocations.");
      } else if (q.toLowerCase().includes("scheme") || q.toLowerCase().includes("incentive")) {
        setAiChatResponse("AI Policy Opportunity Signal: MHI announced $1.2B sodium-ion cell PLI subsidies (PLI V2) offering up to 12% raw sourcing CapEx reimbursements.");
      } else {
        setAiChatResponse("AI Policy Summary: Standardized waver exports bilateral clearances are now zero-rated, easing logistics custom schedules across treaty ports.");
      }
    }, 550);
  };
  ...
  const handleCreatePolicyAlert = () => {
    ...
    showToast(`Policy alert created: Notify when new MeitY/MHI rules affect My Sectors ✓`);
  };

  const filteredPolicies = MOCK_POLICIES.filter(pol => {
    const matchesUserPrefs = mySectorPreferences.includes(pol.sectorCode);
    ...
    return matchesUserPrefs && matchesSearch && matchesSector && matchesType;
  });

  const featuredPolicy = MOCK_POLICIES[0];
  ...
  return (
    <div className="bg-gray-50 dark:bg-[#070b12] text-gray-900 dark:text-gray-100 min-h-screen pb-16 transition-colors duration-300">
      ...
      {/* ─── Viksit Bharat Panel Editor Submenu Header Bar ─── */}
      <nav aria-label="Viksit Bharat Panel Editor Submenu Navigation" className="mx-auto max-w-7xl px-4 pt-4 lg:px-6">
        <div className="flex items-center justify-between py-3 px-4 bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xs flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-xs">
              ...
              <span className="text-xs font-bold uppercase tracking-wider">Viksit Bharat Panel Editor Submenu</span>
            </div>
            <span className="text-[11px] font-mono text-gray-400 hidden sm:inline">Panel discussions aligned to Viksit Bharat 2047.</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-gray-900 p-1 rounded-xl border border-gray-200 dark:border-gray-800">
            <Link
              href="/en/poc-v2/headlines/viksit-bharat-panel/my"
            ...
            <Link
              href="/en/poc-v2/headlines/viksit-bharat-panel/all"
      ...
      <nav className="mx-auto max-w-7xl px-4 pt-3 lg:px-6 text-xs text-gray-400 font-semibold flex items-center gap-1.5">
        ...
        <Link href="/en/poc-v2/headlines/viksit-bharat-panel/all" className="hover:text-blue-500">Viksit Bharat Panel Editor</Link>
        <ChevronRight className="h-3 w-3" />
      ...
      <header className="mx-auto max-w-7xl px-4 pt-6 pb-4 lg:px-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 dark:border-gray-800 pb-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-slate-200 dark:bg-slate-800 text-gray-700 dark:text-gray-300 text-[9px] font-bold px-2 py-0.5 rounded font-mono">
                My Sectors Active ({mySectorPreferences.length})
              </span>
            </div>
            
            <h1 className="font-display text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-white leading-tight">
              Policy Intelligence for My Sectors
            </h1>
            <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 font-normal leading-relaxed">
              Track important government policies, reforms, schemes and development initiatives relevant to the sectors you follow — and understand their potential business impact.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 shrink-0 flex-wrap">
            ...
            >
              <Briefcase className="h-4 w-4 text-blue-505" /> Manage My Sectors
            </button>
            <button 
        ...
        <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 p-4 rounded-2xl shadow-3xs flex flex-col md:flex-row items-center gap-4 justify-between">
          <div className="relative w-full md:w-64">
            ...
            <input 
              type="text" 
              placeholder="Search policies, amendments or circulars..."
              value={searchQuery}
      ...
      <nav className="mx-auto max-w-7xl px-4 pt-2 lg:px-6">
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-2 mb-4">
          <h3 className="font-display text-xs font-bold text-gray-400 uppercase tracking-widest">My Followed Sectors</h3>
            ...
            <section className="mx-auto max-w-7xl px-4 pt-6 lg:px-6">
              ...
              <div className="bg-white dark:bg-[#0f172a] border border-gray-205 dark:border-gray-800 p-6 rounded-2xl shadow-sm flex flex-col md:flex-row gap-6 items-start font-sans">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900/60 p-4 rounded-xl border border-gray-150 dark:border-gray-850 text-xs">
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-400 font-normal mt-0.5">{featuredPolicy.whyMatters}</p>
                    </div>
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-400 font-normal mt-0.5">{featuredPolicy.whenApplies}</p>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                    ...
                    {filteredPolicies.map(pol => (
                      <div key={pol.id} className="bg-white dark:bg-[#0f172a] border border-gray-205 dark:border-gray-800 p-5 rounded-2xl shadow-sm space-y-3 group hover:border-blue-500/25 transition-all">
                        <div className="flex justify-between items-center text-[10px] font-bold">
                          ...
                          <span className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded text-gray-600 dark:text-gray-400 font-mono">Status: {pol.status}</span>
                        </div>
                        ...
                        <div className="bg-gray-50 dark:bg-gray-900/60 p-3 rounded-lg border border-gray-150 dark:border-gray-850 space-y-2 text-[11px] font-sans">
                        ...
                        <div className="flex justify-between items-center pt-2 text-[10px] border-t border-gray-55 dark:border-gray-800/40">
                          <span className="text-gray-400 font-semibold font-mono">Source: {pol.source}</span>
                ...
                <div className="space-y-4">
                  <div className="border-b border-gray-200 dark:border-gray-800 pb-2">
                    <h3 className="font-display text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">
                      Policy Tracker Lifecycle
                    </h3>
                  </div>

                  <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 p-5 rounded-2xl shadow-sm space-y-4">
                    {filteredPolicies.map(pol => (
                      <div key={pol.id} className="space-y-2 border-b border-gray-100 dark:border-gray-850/50 pb-3 last:border-0 last:pb-0">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold">
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 p-4 rounded-xl shadow-xs space-y-2">
                      <span className="text-[9px] font-bold text-emerald-500 uppercase">Potential Opportunities</span>
                      <ul className="space-y-1.5 text-gray-600 dark:text-gray-400 font-normal">
                        <li>• Sovereign storage PLI incentives lower GPU local host CapEx costs</li>
                        <li>• Zero-rated wafer machinery customs duty speeds up OSAT packaging setups</li>
                      </ul>
                    </div>

                    <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 p-4 rounded-xl shadow-xs space-y-2">
                      <span className="text-[9px] font-bold text-red-500 uppercase">Compliance Challenges</span>
                      <ul className="space-y-1.5 text-gray-600 dark:text-gray-400 font-normal">
                        <li>• Auditing localized logs carry up to 4% capital reserve compliance rules</li>
                        <li>• Exporters shifting raw sourcing logistics to verify bilateral compliance</li>
                      </ul>
                    </div>
                  </div>
                </div>
                    ...
                    {MOCK_SCHEMES.map(sch => (
                      <div key={sch.id} className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 p-4 rounded-xl shadow-xs space-y-2 text-xs font-semibold">
                        <div className="flex justify-between items-center text-[9px] font-bold">
                          <span className="text-blue-500">{sch.ministry}</span>
                          <span className="text-gray-400">{sch.sectorName}</span>
                        </div>
                        ...
                        <p className="text-gray-600 dark:text-gray-400 font-normal">
                          <strong className="text-[10px] text-gray-900 dark:text-white block">Benefit:</strong> {sch.benefit}
                        </p>
                        <div className="flex justify-between text-[9px] text-gray-405 pt-1.5 border-t border-gray-50 dark:border-gray-850">
              ...
              <div className="space-y-6">
                    ...
                    {[
                      ...
                      <div key={idx} className="space-y-1">
                        <div className="flex justify-between items-center text-[10.5px]">
                          ...
                          <span className="text-blue-500 font-mono text-[9px]">{row.updates} updates</span>
                        </div>
                ...
                <div className="border border-blue-200/50 dark:border-blue-900/40 bg-blue-50/10 dark:bg-blue-900/5 p-4 rounded-xl shadow-sm space-y-3">
                  <div className="flex justify-between items-center pb-2 border-b border-blue-200/20">
                    <span className="text-[9px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest">Sponsored Awareness Campaign</span>
                    <span className="bg-amber-500 text-gray-950 text-[7px] font-extrabold px-1.5 py-0.5 rounded font-mono">SPONSORED</span>
                  </div>
                  
                  <div className="space-y-1">
                    <span className="text-[8px] font-bold text-blue-500 uppercase font-sans">Semiconductor safety</span>
