...
export default function EOIFormModule({
  ...
  const handleProductSelect = (id: ProductId) => {
    ...
    const newSub = createEmptySubmission(id, product.name, product.platform);
    ...
    if (data.interested_in && data.interested_in !== selectedProduct) {
      ...
      if (product) {
        ...
        if (stored) {
          ...
          stored.meta.platform = product.platform;
  ...
  return (
    ...
    >
      ...
      <div className={`mx-auto ${stage === "select" ? "max-w-6xl" : "max-w-2xl"} transition-all duration-300`}>
