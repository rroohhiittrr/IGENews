          ...
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-12 lg:col-span-8 space-y-8">
              ...
              <div className="space-y-4">
                ...
                <Card className="p-6 border-l-4 border-l-amber-500 bg-amber-50/15 dark:bg-amber-950/5 space-y-4">
                  <div className="space-y-1">
                    <span className="text-[8px] font-mono font-bold text-amber-600 uppercase tracking-widest">
                      CRITICAL BUSINESS CONTEXT
                    </span>
                    <h3 className="font-display text-base font-bold text-gray-900 dark:text-white leading-tight">
                      {iiData.whyItMatters.title}
                    </h3>
                  </div>

                  <div className="space-y-3 text-xs">
                    <div>
                      <span className="font-bold text-gray-400 block uppercase text-[9px]">What Happened</span>
                      <p className="text-gray-700 dark:text-gray-300 font-normal leading-relaxed mt-0.5">
                        {iiData.whyItMatters.explanation}
                      </p>
                    </div>

                    <div className="flex flex-wrap gap-4 pt-2">
                      <div>
                        <span className="font-bold text-gray-450 block uppercase text-[9px]">Who May Be Affected</span>
                        <div className="flex gap-1.5 mt-1 flex-wrap">
                          {iiData.whyItMatters.affected.map((a, i) => (
                            <span key={i} className="bg-gray-100 dark:bg-gray-900 px-2.5 py-0.5 rounded text-[10px] font-semibold text-gray-600 dark:text-gray-400">
                              {a}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                        <span className="font-bold text-gray-455 block uppercase text-[9px]">What To Watch</span>
                        <span className="text-gray-700 dark:text-gray-300 font-medium block mt-1">
                          ⚡ {iiData.whyItMatters.watch}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {/* ── 15. INDUSTRY FORECAST & OUTLOOK ── */}
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-gray-250 dark:border-gray-800 pb-3">
                  <h3 className="font-display text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">
                    Industry Forecast & Outlook Scenarios
                  </h3>
                  
                  {/* Scenario Toggles */}
                  <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-900 p-1 rounded-xl text-[10px] font-bold">
                    {(["Base Case", "High Growth", "Downside"] as const).map(sc => (
                      <button
                        key={sc}
                        onClick={() => setIiActiveScenario(sc)}
                        className={`px-2.5 py-1 rounded-lg transition-all ${
                          iiActiveScenario === sc 
                            ? "bg-white dark:bg-gray-800 text-amber-600 shadow-xs" 
                ...
                <Card className="p-5 space-y-4">
                  <div className="grid grid-cols-4 gap-2 text-center">
                    {[
                      { label: "Current", value: "Active", border: "border-r border-gray-100 dark:border-gray-800" },
                      { label: "2027 Proj", value: iiData.forecast.c2027, border: "border-r border-gray-100 dark:border-gray-800" },
                      { label: "2028 Proj", value: iiData.forecast.c2028, border: "border-r border-gray-100 dark:border-gray-800" },
                      { label: "2030 Outlook", value: iiData.forecast.c2030, border: "" }
                    ].map((step, i) => (
                      <div key={i} className={step.border}>
                        <span className="text-[9px] text-gray-400 block">{step.label}</span>
                        <span className="text-sm font-bold text-gray-950 dark:text-white block mt-0.5">
                          {iiActiveScenario === "High Growth" && step.label.includes("Proj") 
                            ? (parseInt(step.value.replace("$", "").replace("B", "")) * 1.25).toFixed(0) + "B" 
                            : iiActiveScenario === "Downside" && step.label.includes("Proj")
                            ? (parseInt(step.value.replace("$", "").replace("B", "")) * 0.75).toFixed(0) + "B"
                            : step.value}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="pt-4 border-t border-gray-100 dark:border-gray-800 text-xs space-y-2">
                    <div>
                      <span className="font-bold text-gray-400 block uppercase text-[9px]">Assumptions ({iiActiveScenario})</span>
                      <ul className="list-disc pl-4 space-y-1 text-gray-650 dark:text-gray-300 mt-1 font-normal">
                        {iiData.forecast.assumptions.map((ass, i) => (
                          <li key={i}>{ass} {iiActiveScenario === "High Growth" ? "(with accelerated bilateral subsidy inflows)" : iiActiveScenario === "Downside" ? "(with tariff escalations)" : ""}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <span className="font-bold text-gray-400 block uppercase text-[9px]">Limitations & Methodology</span>
                      <p className="text-gray-500 italic mt-0.5 text-[11px] leading-relaxed">
                        Assumptions based on {iiData.forecast.limitations}. AI-generated projections for alternative scenarios marked as scenarios and not historical fact.
                      </p>
                    </div>
                  </div>
                </Card>
            ...
            <div className="col-span-12 lg:col-span-4 space-y-6">
              ...
              <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-3xl p-5 shadow-xs space-y-3">
                ...
                <p className="text-[11px] text-gray-500 leading-normal font-normal">
                  Share your organization's capacity updates, industrial findings, and research projections with IGEN.
                </p>
                {iiPublishSuccess ? (
                  <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold rounded-xl text-center">
                    ✓ Submitted successfully for review!
                  </div>
                ) : (
                  <div className="space-y-2">
                    <input
                      value={iiPublishCompany}
                      onChange={(e) => setIiPublishCompany(e.target.value)}
                    ...
                    />
                    <input
                      value={iiPublishTitle}
                      onChange={(e) => setIiPublishTitle(e.target.value)}
                      placeholder="Story Headline (e.g. Scaling Solar Assembly)"
                      className="w-full rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-3 py-2 text-xs outline-none focus:border-amber-500"
                    />
                    <button
                      onClick={() => {
                        if (iiPublishCompany && iiPublishTitle) setIiPublishSuccess(true);
                      }}
