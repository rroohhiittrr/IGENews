...
const ARTICLE_DATA = {
  ...
  tags: {
    ...
    leaders: ["Jensen Huang", "Nandan Nilekani", "Dr. Arjan Mehta"],
    readers: ["Procurement Managers", "Export Logistics Leads", "Sourcing Directors"]
  }
};
    ...
    <div className="bg-gray-50 dark:bg-[#070b12] text-gray-900 dark:text-gray-100 min-h-screen pb-16 transition-colors duration-300">
      ...
      <div className="border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0f172a]">
        <div className="mx-auto max-w-7xl px-4 py-3 lg:px-6 flex items-center gap-2 text-xs text-gray-500">
          <Link href="/en/news-poc" className="hover:text-blue-600 flex items-center gap-1">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to News Feed
          </Link>
          <span>/</span>
          <Link href="/en/news-poc/sector-news" className="hover:text-blue-600">Sector News</Link>
          <span>/</span>
      ...
      <div className="mx-auto max-w-7xl px-4 pt-8 lg:px-6">
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            ...
            {/* ── TAXONOMY MAPPING SIDEBAR PANELS ── */}

            {/* 1. Sector Tags */}
            <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-xs space-y-2.5">
              <span className="text-[9px] font-bold text-blue-600 uppercase tracking-widest block">Sectors Tagged</span>
              <div className="flex flex-wrap gap-1.5">
                {ARTICLE_DATA.tags.sectors.map((s, idx) => (
                  <Link key={idx} href="/en/news-poc/sector-news" className="bg-blue-50 dark:bg-blue-950/40 text-blue-600 text-xs font-bold px-2.5 py-1 rounded-lg hover:bg-blue-100 transition-colors">
                    {s}
                  </Link>
                ))}
              </div>
            ...
            {/* 2. Industry Tags */}
            <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-xs space-y-2.5">
              <span className="text-[9px] font-bold text-purple-600 uppercase tracking-widest block">Industries Tagged</span>
              <div className="flex flex-wrap gap-1.5">
                {ARTICLE_DATA.tags.industries.map((ind, idx) => (
                  <span key={idx} className="bg-purple-50 dark:bg-purple-950/40 text-purple-600 text-xs font-bold px-2.5 py-1 rounded-lg">
                    {ind}
                  </span>
                ))}
              </div>
            ...
            {/* 3. Country Tags */}
            <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-xs space-y-2.5">
              <span className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest block">Countries & Corridors Tagged</span>
              <div className="flex flex-wrap gap-1.5">
                {ARTICLE_DATA.tags.countries.map((c, idx) => (
                  <Link key={idx} href="/en/news-poc/country-news" className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 text-xs font-bold px-2.5 py-1 rounded-lg hover:bg-emerald-100 transition-colors">
                    {c}
                  </Link>
                ))}
              </div>
            ...
            {/* 4. Leader Tags */}
            <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-xs space-y-2.5">
              <span className="text-[9px] font-bold text-amber-600 uppercase tracking-widest block">Leaders & C-Suite Mentioned</span>
              <div className="flex flex-wrap gap-1.5">
                {ARTICLE_DATA.tags.leaders.map((l, idx) => (
                  <Link key={idx} href="/en/news-poc/leader-news" className="bg-amber-50 dark:bg-amber-950/40 text-amber-600 text-xs font-bold px-2.5 py-1 rounded-lg hover:bg-amber-100 transition-colors">
                    👤 {l}
                  </Link>
                ))}
              </div>
            ...
            {/* 5. Reader Profile Targets */}
            <div className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-xs space-y-2.5">
              <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest block">Recommended Reader Groups</span>
              <div className="flex flex-wrap gap-1.5">
                {ARTICLE_DATA.tags.readers.map((r, idx) => (
                  <span key={idx} className="bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300 text-xs font-bold px-2.5 py-1 rounded-lg">
                    {r}
                  </span>
                ))}
              </div>
            ...
            {/* ── STAY UPDATED NEWSLETTER BOX (MOVED TO BOTTOM OF RHS SIDEBAR) ── */}
        ...
        <section className="pt-12 space-y-4">
          <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-3">
            ...
            <Link href="/en/news-poc" className="text-xs font-bold text-blue-600 hover:underline">View All News →</Link>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {RELATED_INTEL.map((item) => (
              <Link key={item.id} href={`/en/news-poc/article/${item.id}`} className="bg-white dark:bg-[#0f172a] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-xs hover:border-blue-500 transition-all group block">
