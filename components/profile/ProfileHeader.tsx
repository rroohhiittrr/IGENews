  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 right-6 flex items-center gap-3">
        ...
        <button className="text-gray-400 hover:text-[#F4A024] transition-colors">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <div className="flex-1 text-center md:text-left space-y-4 pt-2">
           <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-2">
              <h2 className="text-2xl font-bold text-[#1E3A5F] dark:text-white">{user.name || "User Name"}</h2>
              {!statusInfo.isFree && (
                 <span className="px-2 py-0.5 bg-blue-50 text-[#1E3A5F] dark:bg-blue-950/20 dark:text-blue-400 text-[10px] font-black rounded-lg border border-blue-200 dark:border-transparent uppercase tracking-wider flex items-center gap-1 shrink-0">
                    <BadgeCheck className="w-4 h-4 text-emerald-500 shrink-0" /> Verified
                 </span>
              )}
           </div>
           
           <span className="inline-block text-[10px] font-bold text-[#F4A024] bg-[#F4A024]/10 px-3 py-1 rounded-full uppercase tracking-widest">
              {statusInfo.label}
           </span>
           ...
           <div className="space-y-2 text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto md:mx-0">
              <div className="flex flex-col sm:flex-row sm:gap-2">
                 ...
                 <span className="text-[#1E3A5F] dark:text-white font-medium text-left">24 November 2026</span>
              </div>
              <div className="flex flex-col sm:flex-row sm:gap-2">
                 <span className="font-medium text-gray-400 sm:w-36 text-left">Country, city:</span>
                 <span className="text-[#1E3A5F] dark:text-white font-medium text-left">{user.countries?.[0] || "Not set"}</span>
              </div>
              <div className="flex flex-col sm:flex-row sm:gap-2">
                 <span className="font-medium text-gray-400 sm:w-36 text-left">E-mail:</span>
                 <span className="text-[#1E3A5F] dark:text-white font-medium text-left">{user.email}</span>
              </div>
              {user.mobile && (
                 <div className="flex flex-col sm:flex-row sm:gap-2">
                    <span className="font-medium text-gray-400 sm:w-36 text-left">Phone:</span>
                    <span className="text-[#1E3A5F] dark:text-white font-medium text-left">{user.mobile}</span>
                 </div>
              )}
              <div className="flex flex-col sm:flex-row sm:gap-2">
                 <span className="font-medium text-gray-400 sm:w-36 text-left">Public Profile URL:</span>
                 <span className="text-[#1E3A5F] dark:text-white font-mono text-xs font-semibold text-left select-all truncate">{publicUrl}</span>
              </div>
           </div>
