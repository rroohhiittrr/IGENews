...
// ─── Public Profile Card (SME Elite Emerald) ─────────────
function SMEElitePublicProfile({
...
export default function SMEEliteDashboard() {
  ...
  const [editDesignation, setEditDesignation] = useState(profile.currentDesignation || "Senior Trade Consultant & Policy Fellow");
  const [editOrg, setEditOrg] = useState(profile.organisation || "Independent Specialist Advisory");
  const [editCity, setEditCity] = useState(profile.city || "Mumbai");
  const [editCountry, setEditCountry] = useState(profile.country || "India");
  const [editBio, setEditBio] = useState(profile.bio || "Senior trade specialist with 20+ years expertise advising leadership on bilateral FTAs and tariff mitigations.");
  ...
  const totalArticleRevenue = articles.reduce((a, b) => a + b.revenue, 0);
  ...
  const handlePublish = () => {
    ...
    if (articles.length >= 6) {
      alert("Elite Tier limit reached (6 articles/month max). Upgrade to SME Sovereign for 8 articles/month.");
      return;
    }
  ...
  return (
    <div className="p-5 md:p-8 lg:p-10 max-w-5xl mx-auto pb-24">
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "Elite Articles", value: articles.length, icon: FileText, color: "from-emerald-600 to-teal-500" },
              { label: "Total Reads", value: articles.reduce((a, b) => a + b.reads, 0).toLocaleString(), icon: Eye, color: "from-blue-600 to-indigo-500" },
              { label: "Profile Views", value: "3,480", icon: User, color: "from-purple-600 to-violet-500" },
              { label: "Article Revenue", value: `₹${totalArticleRevenue.toFixed(0)}`, icon: TrendingUp, color: "from-amber-500 to-yellow-500" },
            ].map((s, i) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white dark:bg-[#122238] rounded-2xl p-5 border border-gray-100 dark:border-white/5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-sm text-[#1D1D46] dark:text-white flex items-center gap-2">
                    <FileText className="w-4 h-4 text-emerald-500" /> Published Trade Analyses
                  </h3>
            ...
            <div className="space-y-4">
              <div className="bg-white dark:bg-[#122238] rounded-2xl p-5 border border-gray-100 dark:border-white/5 shadow-sm space-y-3">
                <h3 className="font-bold text-sm text-[#1D1D46] dark:text-white flex items-center gap-2">
                  <Award className="w-4 h-4 text-emerald-500" /> Elite Authority Standing
                </h3>
                <div className="text-xs text-gray-500 space-y-2">
                  <div className="flex justify-between py-1 border-b border-gray-50 dark:border-white/5">
                    <span>Directory Position</span>
                    <span className="font-bold text-[#1D1D46] dark:text-white">Top 5% in Sector</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-50 dark:border-white/5">
                    <span>Global Search Rank</span>
                    <span className="font-bold text-emerald-600">#4 National</span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span>Membership Status</span>
                    <span className="font-bold text-emerald-600">SME Elite (Active)</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {activeTab === "articles" && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-[#1D1D46] dark:text-white">SME Elite Column & Articles</h2>
              <p className="text-xs text-gray-400 mt-0.5">Publish thought leadership articles and industry commentaries</p>
            </div>
      ...
      {/* ── 3. INSIGHTS TAB ── */}
      {activeTab === "insights" && (
        <SmeAnalyticsHub currentTier="elite" role="sme" userName={displayName} />
      )}
      ...
      {activeTab === "settings" && (
        <div className="bg-white dark:bg-[#122238] rounded-2xl p-6 border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Manage your verified expert credentials and public information</p>
          </div>
          ...
          <form onSubmit={handleSaveSettings} className="space-y-4 text-xs">
            ...
            <div>
              <label className="block text-gray-600 dark:text-gray-300 font-bold mb-1">Expert Bio & Editorial Statement</label>
              <textarea
                rows={3}
                value={editBio}
