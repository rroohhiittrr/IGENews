...
export default function InterestsCard({ user }: InterestsCardProps) {
  return (
    ...
    >
      <h2 className="text-xl font-bold text-[#1D1D46] mb-6">Identity Details</h2>

      <div className="space-y-6">
         <div>
            <p className="text-sm font-medium text-gray-500 mb-2">Primary Role:</p>
            <div className="bg-[#f4f7fb] px-4 py-3 rounded-2xl text-sm font-bold text-[#1D1D46] capitalize shadow-inner">
         ...
         <div>
            ...
            <div className="flex flex-wrap gap-2">
               {user.sectors?.slice(0, 3).map(s => (
                  <span key={s} className="px-3 py-1.5 bg-[#1D1D46] text-white text-xs font-bold rounded-xl shadow-sm capitalize">
                     {s.replace("-", " ")}
                  </span>
               ))}
               {user.sectors?.length > 3 && (
                  <span className="px-3 py-1.5 bg-[#f4f7fb] text-[#1D1D46] text-xs font-bold rounded-xl shadow-sm">
                     +{user.sectors.length - 3}
                  </span>
               )}
            </div>
         ...
         <div>
            ...
            <div className="flex flex-wrap gap-2">
               {user.countries?.slice(0, 3).map(c => (
                  <span key={c} className="px-3 py-1.5 bg-[#F0652E] text-white text-xs font-bold rounded-xl shadow-sm">
                     {c}
                  </span>
               ))}
