      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-[28px] p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-[#1D1D46] text-sm">{isVerified ? "Verified Identity" : "Unverified"}</h3>
                <p className="text-xs text-gray-500">{isVerified ? "Your profile is verified" : "Apply for verification"}</p>
              </div>
            </div>
            {!isVerified && (
              <button className="w-full py-2.5 bg-[#1D1D46] text-white text-sm font-bold rounded-xl hover:bg-[#F0652E] transition-colors">
          ...
          <div className="bg-white rounded-[28px] p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-[#1D1D46]">Reputation Score</h3>
              <span className="text-2xl font-black text-[#F0652E]">740</span>
            </div>
            <div className="h-3 w-full bg-[#f4f7fb] rounded-full overflow-hidden mb-2">
              <motion.div initial={{ width: 0 }} animate={{ width: "74%" }} transition={{ duration: 1.2, delay: 0.3 }}
                className="h-full bg-gradient-to-r from-[#1D1D46] to-[#F0652E] rounded-full" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[["Articles","342"],["Comments","42"],["Likes","856"],["Reports","8"]].map(([l,v]) => (
                <div key={l} className="bg-[#f4f7fb] rounded-xl p-3 text-center">
                  <div className="text-lg font-black text-[#1D1D46]">{v}</div>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-[28px] p-6 md:p-8 shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-[#1D1D46] mb-8 flex items-center gap-2">
              <Trophy className="w-5 h-5 text-[#F0652E]" /> Growth Journey
            </h3>
            <div className="relative pl-8">
              <div className="absolute left-3 top-4 bottom-4 w-0.5 bg-[#1D1D46]/15 rounded-full" />
              <div className="space-y-5">
                {stages.map((s) => (
                  <div key={s.n} className={`relative flex gap-5 ${!s.active ? "opacity-40" : ""}`}>
                    <div className={`absolute -left-8 z-10 w-6 h-6 rounded-full border-4 border-[#f4f7fb] shadow-md ${s.done ? "bg-[#F0652E]" : s.active ? "bg-[#1D1D46]" : "bg-gray-300"}`} />
                    <div className={`flex-1 rounded-2xl p-5 ${s.done ? "bg-[#f4f7fb]" : s.active ? "bg-[#1D1D46]/5 border border-[#1D1D46]/10" : "bg-gray-50"}`}>
                      <div className="flex items-center justify-between mb-1 gap-4 flex-wrap">
                        <h4 className={`font-bold text-base ${s.active ? "text-[#1D1D46]" : "text-gray-400"}`}>
                          Stage {s.n}: {s.title}
                        </h4>
                        {s.done
          ...
          {/* Badges */}
          <div className="bg-white rounded-[28px] p-6 md:p-8 shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-[#1D1D46] mb-6 flex items-center gap-2">
              <Medal className="w-5 h-5 text-[#F0652E]" /> Achievements & Badges
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {badges.map((b) => (
                <div key={b.label} className={`rounded-2xl p-4 border text-center hover:shadow-md transition-all ${b.earned ? `${b.bg} ${b.border}` : "bg-gray-50 border-gray-100"}`}>
                  ...
                  <p className={`text-xs font-bold leading-snug ${b.earned ? "text-[#1D1D46]" : "text-gray-400"}`}>{b.label}</p>
