...
export default function AssociateSMEProDashboard() {
  ...
  const [editDesignation, setEditDesignation] = useState(profile.currentDesignation || "Associate Trade Specialist");
  const [editOrg, setEditOrg] = useState(profile.organisation || "Logistics & Sourcing Consultant");
  const [editCity, setEditCity] = useState(profile.city || "Mumbai");
  const [editCountry, setEditCountry] = useState(profile.country || "India");
  const [editBio, setEditBio] = useState(profile.bio || "Associate trade specialist supporting cross-border supply chain operations, MSME export compliance, and logistics optimization.");
  ...
  return (
    <div className="p-5 md:p-8 lg:p-10 max-w-5xl mx-auto pb-24">
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "Articles Quota", value: `${articles.length}/4`, icon: FileText, color: "from-blue-600 to-blue-500" },
              { label: "Total Reads", value: articles.reduce((a, b) => a + b.reads, 0).toLocaleString(), icon: Eye, color: "from-indigo-600 to-indigo-500" },
              { label: "Profile Views", value: "512", icon: User, color: "from-emerald-600 to-teal-500" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white dark:bg-[#122238] rounded-2xl p-5 border border-gray-100 dark:border-white/5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-sm text-[#1D1D46] dark:text-white flex items-center gap-2">
                    <FileText className="w-4 h-4 text-blue-500" /> Recent Published Articles
                  </h3>
            ...
            <div className="space-y-4">
              <div className="bg-white dark:bg-[#122238] rounded-2xl p-5 border border-gray-100 dark:border-white/5 shadow-sm space-y-3">
                <h3 className="font-bold text-sm text-[#1D1D46] dark:text-white flex items-center gap-2">
                  <Award className="w-4 h-4 text-amber-500" /> Associate Authority Status
                </h3>
                <div className="text-xs text-gray-500 space-y-2">
                  <div className="flex justify-between py-1 border-b border-gray-50 dark:border-white/5">
                    <span>Directory Position</span>
                    <span className="font-bold text-[#1D1D46] dark:text-white">Active Contributor</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-50 dark:border-white/5">
                  ...
                  <div className="flex justify-between py-1">
                    <span>Plan Tier</span>
                    <span className="font-bold text-blue-600">ASME Pro (Active)</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {activeTab === "articles" && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-[#1D1D46] dark:text-white">Your Articles ({articles.length}/4 this month)</h2>
              <p className="text-xs text-gray-400 mt-0.5">Share practical industry notes and trade insights</p>
            </div>
            {articles.length < 4 && (
      ...
      {/* ── 3. INSIGHTS TAB ── */}
      {activeTab === "insights" && (
        <SmeAnalyticsHub currentTier="pro" role="associate-sme" userName={displayName} />
      )}
      ...
      {activeTab === "settings" && (
        <div className="bg-white dark:bg-[#122238] rounded-2xl p-6 border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Manage your verified Associate profile details and preferences</p>
          </div>
          ...
          <form onSubmit={handleSaveSettings} className="space-y-4 text-xs">
            ...
            <div>
              <label className="block text-gray-600 dark:text-gray-300 font-bold mb-1">Bio / Profile Summary</label>
              <textarea
                rows={3}
                value={editBio}
