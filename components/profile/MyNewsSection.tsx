  ...
  return (
    ...
    >
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
        <h2 className="text-2xl font-bold text-[#1D1D46] whitespace-nowrap">
          News Hub
        </h2>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${
                  activeTab === tab.id 
                    ? "bg-white text-[#1D1D46] shadow-sm" 
                    : "text-gray-500 hover:text-[#1D1D46] hover:bg-white/50"
                }`}
              >
                <tab.icon className={`h-4 w-4 ${activeTab === tab.id ? "fill-[#F0652E] text-[#F0652E]" : ""}`} />
                <span>{tab.label}</span>
              </button>
            ))}
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {getActiveArticles().map((article) => (
              <div 
                key={article.id} 
                className="group relative flex flex-col rounded-[24px] border border-gray-100 overflow-hidden hover:shadow-xl hover:border-[#1D1D46]/20 transition-all duration-300 bg-white"
              >
                {/* Image */}
                <div className="h-48 w-full overflow-hidden relative">
                  ...
                  />
                  <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-xl text-xs font-bold text-[#1D1D46] shadow-sm">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <h3 className="text-lg font-bold text-[#1D1D46] mb-4 line-clamp-2 leading-snug group-hover:text-[#F0652E] transition-colors">
                    {article.title}
                  </h3>
                  ...
                  {activeTab === 'sme-articles' && (
                     <div className="flex items-center gap-3 mb-4">
                         <div className="h-8 w-8 rounded-full bg-[#f4f7fb] flex items-center justify-center text-[#1D1D46] text-[10px] font-bold">
                             SME
                         </div>
                  ...
                  <div className="mt-auto flex items-center justify-between text-sm text-gray-400 pt-4 border-t border-gray-100">
                    ...
                    <div className="flex gap-4">
                      <button className="hover:text-[#F0652E] transition-all"><Share2 className="h-5 w-5" /></button>
                      <button className="hover:text-[#F0652E] transition-all">
                        <Bookmark className={`h-5 w-5 ${activeTab === 'saved' ? 'fill-[#F0652E] text-[#F0652E]' : ''}`} />
                      </button>
                    </div>
                  </div>
                </div>
      ...
      <div className="mt-10 text-center">
        <Link 
          href="/my-news" 
          className="inline-flex items-center gap-2 text-sm font-bold text-white bg-[#1D1D46] hover:bg-[#F0652E] shadow-md hover:shadow-lg transition-all py-3 px-8 rounded-2xl"
        >
