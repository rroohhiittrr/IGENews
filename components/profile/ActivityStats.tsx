...
export default function ActivityStats() {
  const stats = [
    { label: "Articles Read", value: "342", icon: BookOpen, color: "text-[#1D1D46]", bg: "bg-[#f4f7fb]", trend: "+24%" },
    { label: "Bookmarks", value: "124", icon: Bookmark, color: "text-[#F0652E]", bg: "bg-orange-50", trend: "+12%" },
  ...
  return (
    <div className="bg-white rounded-[32px] p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] h-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <h2 className="text-2xl font-bold text-[#1D1D46]">Overview</h2>
        <span className="text-xs font-bold px-4 py-2 rounded-xl bg-[#f4f7fb] text-[#1D1D46]">
          Last 30 Days
        </span>
      </div>
      ...
      >
        {stats.map((stat, index) => (
          ...
          >
            ...
            <div>
              <h3 className="text-3xl font-black text-[#1D1D46] tracking-tight">{stat.value}</h3>
