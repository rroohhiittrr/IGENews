  ...
  return (
    <div className="p-5 md:p-8 lg:p-10">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[#1E3A5F]" style={{ fontFamily: "var(--font-display)" }}>My News</h1>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="pl-11 pr-4 py-2.5 rounded-full bg-white shadow-sm text-sm w-full border-none focus:outline-none focus:ring-2 focus:ring-[#F4A024] transition-all"
          />
        </div>
      </div>
            ...
            <button
              ...
              className={`flex items-center gap-2.5 px-5 py-3 rounded-2xl text-sm font-bold whitespace-nowrap transition-all ${
                activeTab === tab.id
                  ...
                  : "bg-white text-gray-500 hover:text-[#1E3A5F] hover:shadow-md shadow-sm"
              }`}
            >
              <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? "text-[#F4A024]" : ""}`} />
              {tab.label}
              {tab.count && (
                <span className={`text-xs px-2 py-0.5 rounded-full font-black ${activeTab === tab.id ? "bg-white/20 text-white" : "bg-[#f4f7fb] text-[#1E3A5F]"}`}>
                  {tab.count}
                </span>
              )}
            </button>
          ...
          {articles.length > 0 ? articles.map(article => (
            <div key={article.id} className="group relative bg-white rounded-[24px] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100">
              <div className="h-48 overflow-hidden relative">
                ...
                <div className="absolute top-3 right-3 bg-white/95 px-3 py-1 rounded-xl text-xs font-bold text-[#1E3A5F] shadow">
                  {article.sector?.name || article.tags?.[0] || "News"}
                </div>
                {activeTab === "sme-articles" && (
                  <div className="absolute top-3 left-3 bg-[#F4A024] text-white text-[10px] font-black px-2.5 py-1 rounded-lg shadow">
                    SME
                  </div>
                )}
              </div>
              <div className="p-5">
                <h3 className="font-bold text-[#1E3A5F] text-base line-clamp-2 leading-snug group-hover:text-[#F4A024] transition-colors mb-3">
                  {article.title}
                </h3>
                <div className="flex items-center justify-between text-xs text-gray-400 pt-3 border-t border-gray-100">
                  ...
                  <div className="flex gap-3">
                    <button className="hover:text-[#F4A024] transition-colors"><Share2 className="w-4 h-4" /></button>
                    <button className="hover:text-[#F4A024] transition-colors"><Bookmark className={`w-4 h-4 ${activeTab === "saved" ? "fill-[#F4A024] text-[#F4A024]" : ""}`} /></button>
                  </div>
                </div>
              </div>
