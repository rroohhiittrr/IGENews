...
export default function AuthorityCard({ user }: AuthorityCardProps) {
  ...
  const stages = [
    { id: 1, title: "Reader — Consuming intelligence", subtitle: "Reading, bookmarking, and engaging with content.", active: true, completed: true, color: "bg-[#f4f7fb]", textColor: "text-[#1D1D46]", date: "" },
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold text-[#1D1D46] mb-8">Growth Journey</h2>
      
      <div className="relative pl-4 md:pl-6">
        {/* Vertical Line */}
        <div className="absolute left-[23px] md:left-[31px] top-6 bottom-6 w-0.5 bg-[#1D1D46]/20"></div>

        <div className="space-y-6">
           {stages.map((stage) => (
              <div key={stage.id} className="relative flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                 {/* Circle Marker */}
                 <div className="relative z-10 w-4 h-4 rounded-full border-2 border-[#1D1D46] bg-white shadow-[0_0_0_4px_white] shrink-0 self-start md:self-center mt-6 md:mt-0">
                    {stage.completed && <div className="absolute inset-[3px] bg-[#1D1D46] rounded-full"></div>}
                 </div>
                 ...
                 <div className={`flex-1 rounded-2xl p-5 ${stage.color} relative group transition-all hover:shadow-md`}>
                    <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4 pr-10">
                       ...
                       {stage.completed ? (
                          ...
                          <span className="px-3 py-1 bg-[#1D1D46] text-white text-xs font-bold rounded-lg shadow-sm shrink-0 self-start">
                             {stage.date}
                          </span>
                       ) : null}
                    </div>
                    ...
                    {/* Arrow Button */}
                    <button className="absolute bottom-4 right-4 w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#1D1D46] shadow-sm opacity-50 group-hover:opacity-100 transition-opacity">
