...
import {
  ...
  Microchip, Pickaxe, TestTube, Sparkles, AlertTriangle,
  type LucideIcon,
} from "lucide-react";
...
function ProgressBar({ step }: { step: number }) {
  const steps = ["Sectors & Industries", "Bilateral Countries", "Leaders"];
  return (
...
/* ============================
   STEP 2 — COUNTRIES (up to 10 total: max 2 top 10, max 8 other)
   ============================ */
function CountryStep({ selected, onToggle }: { selected: string[]; onToggle: (name: string) => void }) {
  ...
  return (
    <div className="onb-step">
      ...
      {/* Error / Out of selections Toast Alert */}
      <AnimatePresence>
        {errorMsg && (
          <motion.div
