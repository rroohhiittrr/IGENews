...
export default function ProfileSidebar() {
  ...
  return (
    <div className="w-64 bg-[#1D1D46] rounded-r-[40px] h-[calc(100vh)] p-6 text-white flex-col relative overflow-hidden hidden md:flex sticky top-0 shadow-2xl z-10 border-r border-[#121232]">
          ...
          return (
            <Link
              ...
              className={`flex items-center justify-between px-5 py-3.5 rounded-l-3xl transition-all w-[calc(100%+24px)] ${
                isActive 
                  ? "bg-[#f4f7fb] text-[#1D1D46] font-bold shadow-lg relative" 
            ...
            >
              ...
              {item.badge && (
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full z-10 ${isActive ? "bg-[#1D1D46] text-white" : "bg-white/20 text-white"}`}>
                  {item.badge}
                </span>
              )}
              {isActive && (
