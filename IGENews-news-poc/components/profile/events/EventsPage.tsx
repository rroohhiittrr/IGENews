...
export default function EventsPage() {
  ...
  return (
    <div className="p-5 md:p-8 lg:p-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-[#1D1D46]" style={{ fontFamily: "var(--font-display)" }}>Events</h1>
        <p className="text-sm text-gray-500 mt-1">Trade summits, webinars, and industry networking events</p>
      </div>

      {/* My Registered Events */}
      <section className="mb-10">
        <h2 className="text-base font-bold text-[#1D1D46] mb-4 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-[#F0652E]" /> My Registered Events
          <span className="ml-auto text-xs font-bold px-3 py-1 bg-[#F0652E] text-white rounded-full">{upcoming.length}</span>
        </h2>
            ...
            return (
              <motion.div key={event.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <h3 className="font-bold text-[#1D1D46] text-base leading-snug">{event.title}</h3>
                  <span className={`text-[10px] font-black px-3 py-1 rounded-lg shrink-0 ${tc.bg} ${tc.text}`}>{event.type}</span>
                </div>
                <div className="space-y-2 text-sm text-gray-500">
                  <div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-[#F0652E] shrink-0" />{event.date}</div>
                  <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-[#F0652E] shrink-0" />{event.time}</div>
                  <div className="flex items-center gap-2">{event.isVirtual ? <Globe className="w-4 h-4 text-[#F0652E] shrink-0" /> : <MapPin className="w-4 h-4 text-[#F0652E] shrink-0" />}{event.location}</div>
                  <div className="flex items-center gap-2"><Users className="w-4 h-4 text-[#F0652E] shrink-0" />{event.attendees.toLocaleString()} attendees</div>
                </div>
                <div className="mt-5 flex gap-3">
                  <button className="flex-1 py-2.5 bg-[#1D1D46] text-white text-sm font-bold rounded-xl hover:bg-[#F0652E] transition-colors">
                    Join Event
                  </button>
                  <button className="px-4 py-2.5 bg-[#f4f7fb] text-[#1D1D46] text-sm font-bold rounded-xl hover:bg-gray-200 transition-colors">
                    Details
                  </button>
                </div>
              </motion.div>
      ...
      <section>
        <h2 className="text-base font-bold text-[#1D1D46] mb-4 flex items-center gap-2">
          <Globe className="w-4 h-4 text-[#1D1D46]" /> Discover Events
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {discover.map((event, i) => {
            ...
            return (
              <motion.div key={event.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.1 }}
                ...
                <button className="mt-5 w-full py-2.5 border-2 border-[#1D1D46] text-[#1D1D46] text-sm font-bold rounded-xl hover:bg-[#1D1D46] hover:text-white transition-all flex items-center justify-center gap-2">
