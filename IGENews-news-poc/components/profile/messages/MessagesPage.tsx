  ...
  return (
    <div className="p-5 md:p-8 lg:p-10 h-screen flex flex-col">
      <div className="mb-6 flex items-center gap-4">
        {selectedThread && (
          <button onClick={() => setSelectedThread(null)} className="md:hidden w-9 h-9 rounded-xl bg-white shadow-sm flex items-center justify-center text-[#1D1D46]">
            <ArrowLeft className="w-5 h-5" />
          </button>
        )}
        <div>
          <h1 className="text-3xl font-bold text-[#1D1D46]" style={{ fontFamily: "var(--font-display)" }}>Messages</h1>
        ...
        <div className={`w-full md:w-80 shrink-0 flex flex-col gap-3 ${selectedThread ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search messages..."
              className="pl-11 pr-4 py-3 rounded-2xl bg-white w-full text-sm border-none shadow-sm focus:outline-none focus:ring-2 focus:ring-[#F0652E]" />
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 hide-scrollbar">
            {filtered.map(thread => (
              <button key={thread.id} onClick={() => setSelectedThread(thread)}
                className={`w-full flex items-start gap-4 p-4 rounded-[20px] text-left transition-all hover:shadow-md ${selectedThread?.id === thread.id ? "bg-[#1D1D46] text-white shadow-lg" : "bg-white shadow-sm"}`}>
                {/* Avatar */}
                <div className={`w-12 h-12 rounded-2xl shrink-0 flex items-center justify-center font-black text-sm ${selectedThread?.id === thread.id ? "bg-white/20 text-white" : "bg-[#f4f7fb] text-[#1D1D46]"}`}>
                  {thread.name.split(" ").map(n => n[0]).join("").slice(0,2)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`font-bold text-sm truncate ${selectedThread?.id === thread.id ? "text-white" : "text-[#1D1D46]"}`}>{thread.name}</span>
                    <span className={`text-[10px] shrink-0 ml-2 ${selectedThread?.id === thread.id ? "text-white/60" : "text-gray-400"}`}>{thread.time}</span>
                  </div>
                  <p className={`text-[10px] mb-1 ${selectedThread?.id === thread.id ? "text-white/70" : "text-[#F0652E] font-semibold"}`}>{thread.role}</p>
                  <p className={`text-xs truncate ${selectedThread?.id === thread.id ? "text-white/70" : "text-gray-500"}`}>{thread.lastMsg}</p>
                </div>
                {thread.unread > 0 && (
                  <span className={`shrink-0 w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center mt-1 ${selectedThread?.id === thread.id ? "bg-[#F0652E] text-white" : "bg-[#1D1D46] text-white"}`}>
                    {thread.unread}
                  </span>
                )}
              </button>
          ...
          {selectedThread ? (
            <div className="flex flex-col h-full bg-white rounded-[28px] shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="flex items-center gap-4 px-6 py-5 border-b border-gray-100">
                <div className="w-10 h-10 rounded-xl bg-[#f4f7fb] flex items-center justify-center font-black text-sm text-[#1D1D46]">
                  {selectedThread.name.split(" ").map(n => n[0]).join("").slice(0,2)}
                </div>
                <div>
                  <h3 className="font-bold text-[#1D1D46] text-sm">{selectedThread.name}</h3>
                  <p className="text-xs text-[#F0652E] font-semibold">{selectedThread.role}</p>
                </div>
                <button className="ml-auto text-gray-400 hover:text-[#1D1D46]"><MoreVertical className="w-5 h-5" /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 hide-scrollbar">
                {selectedThread.messages.map(msg => (
                  <div key={msg.id} className={`flex ${msg.fromMe ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-[18px] text-sm leading-relaxed ${msg.fromMe ? "bg-[#1D1D46] text-white rounded-br-md" : "bg-[#f4f7fb] text-[#1D1D46] rounded-bl-md"}`}>
                      <p>{msg.text}</p>
              ...
              <div className="p-4 border-t border-gray-100">
                <div className="flex items-center gap-3 bg-[#f4f7fb] rounded-2xl px-4 py-2">
                  <input value={draft} onChange={e => setDraft(e.target.value)}
                    ...
                    className="flex-1 bg-transparent text-sm text-[#1D1D46] border-none focus:outline-none py-2" />
                  <button onClick={() => setDraft("")}
                    className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${draft.trim() ? "bg-[#F0652E] text-white shadow" : "bg-gray-200 text-gray-400"}`}>
            ...
            <div className="flex-1 bg-white rounded-[28px] shadow-sm border border-gray-100 flex items-center justify-center flex-col gap-4">
              <div className="w-16 h-16 rounded-full bg-[#f4f7fb] flex items-center justify-center">
                <Circle className="w-8 h-8 text-[#1D1D46]/20" />
              </div>
