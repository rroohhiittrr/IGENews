...
export default function MyNewsCommentsPage() {
  const articles = mockArticles.slice(0, 2); // Mocking user data
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 lg:px-6">
      <div className="mb-6">
        ...
        <p className="text-[var(--color-neutral-dark)] mt-2">
          Articles where you've left comments.
        </p>
      </div>
            ...
            <div className="space-y-4">
              {articles.map((article) => (
                <div key={article.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                  ...
                  <p className="text-sm text-[var(--color-neutral-dark)] mb-3 text-secondary italic">"Your comment here..."</p>
                </div>
              ))}
            </div>
          ) : (
