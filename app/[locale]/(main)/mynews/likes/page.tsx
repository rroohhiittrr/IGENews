...
export default function MyNewsLikesPage() {
  const articles = mockArticles.slice(3, 6); // Mocking user data
            ...
            <div className="space-y-4">
              {articles.map((article) => (
                <div key={article.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                  ...
                  <p className="text-sm text-[var(--color-neutral-dark)]">{article.summary}</p>
                </div>
              ))}
            </div>
          ) : (
