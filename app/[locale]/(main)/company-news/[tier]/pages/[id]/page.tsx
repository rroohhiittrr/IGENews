  ...
  if (error || !detail) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 text-center space-y-4">
        ...
        <button onClick={() => router.back()} className="text-xs font-bold text-[#1D1D46] dark:text-white underline">
          Go Back
        </button>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
          ...
          <div className="bg-white dark:bg-[#122238] rounded-3xl p-6 md:p-8 border border-gray-100 dark:border-white/5 space-y-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
            <div className="flex flex-col sm:flex-row gap-6 sm:items-center">
              {/* Logo fallback */}
              <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-[#1D1D46] to-[#0642BA] flex items-center justify-center text-white font-black text-2xl shrink-0 shadow-md">
                {detail.logoInitials}
              </div>
              <div className="space-y-2">
                ...
                <h1 className="text-2xl md:text-3xl font-black text-[#1D1D46] dark:text-white" style={{ fontFamily: 'var(--font-display)' }}>
                  {detail.name}
                </h1>
            ...
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 border-l-4 border-[#F0652E] pl-4 italic">
          ...
          {detail.tier === 'registered' ? (
            <div className="bg-white dark:bg-[#122238] rounded-3xl p-6 border border-gray-100 dark:border-white/5 space-y-4 relative overflow-hidden">
              <div className="absolute inset-0 bg-gray-50/70 dark:bg-[#0c1829]/70 backdrop-blur-[2px] flex flex-col items-center justify-center text-center p-4 z-10">
                ...
                <h4 className="text-xs font-black text-[#1D1D46] dark:text-white uppercase tracking-wider">Team Leadership Locked</h4>
                ...
                <Link href={`/${locale}/profile/plans/company`} className="mt-3 text-[10px] font-black uppercase text-[#F0652E] hover:underline">
                  Unlock Team linking
                </Link>
              </div>
              <h3 className="text-base font-black text-[#1D1D46] dark:text-white" style={{ fontFamily: 'var(--font-display)' }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {detail.leadership.map((l, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-white/5 rounded-2xl">
                    <div className="w-10 h-10 rounded-xl bg-gray-200 dark:bg-[#1D1D46]/20 text-[#1D1D46] dark:text-white font-bold text-xs flex items-center justify-center shrink-0">
                      {l.initials}
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-[#1D1D46] dark:text-white leading-tight">{l.name}</h4>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-white dark:bg-[#122238] rounded-3xl p-6 border border-gray-100 dark:border-white/5 space-y-4 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
            <h3 className="text-sm font-black text-[#1D1D46] dark:text-white uppercase tracking-wider">
              Profile Summary
            </h3>
            <div className="space-y-3.5 text-xs font-semibold">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[#1D1D46] dark:text-white font-bold">{detail.foundedYear}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400 flex items-center gap-1.5">
                ...
                <span className="text-[#1D1D46] dark:text-white font-bold">{detail.employees}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400 flex items-center gap-1.5">
                ...
                <span className="text-[#1D1D46] dark:text-white font-bold">{detail.revenue}</span>
              </div>
              ...
              <div className="space-y-2.5 pt-1.5">
                {detail.website && (
                  <a
                    ...
                    className="flex items-center gap-2 text-xs font-bold text-[#F0652E] hover:text-[#1D1D46] dark:hover:text-white transition-colors"
                  >
          ...
          <div className="bg-white dark:bg-[#122238] rounded-3xl p-6 border border-gray-100 dark:border-white/5 space-y-4 relative overflow-hidden">
            {detail.tier === 'registered' && (
              <div className="absolute inset-0 bg-gray-50/70 dark:bg-[#0c1829]/70 backdrop-blur-[2px] flex flex-col items-center justify-center text-center p-4 z-10">
                ...
                <h4 className="text-xs font-black text-[#1D1D46] dark:text-white uppercase tracking-wider">B2B Analytics Locked</h4>
            ...
            <h3 className="text-sm font-black text-[#1D1D46] dark:text-white uppercase tracking-wider flex items-center gap-1.5">
              <TrendingUp className="w-4 h-4 text-[#F0652E]" />
              B2B Analytics
            </h3>
            ...
            <div className={`space-y-3 ${detail.tier === 'registered' ? 'opacity-20' : ''}`}>
              ...
              <div className="flex justify-between items-center text-xs font-semibold">
                ...
                <span className="text-[#1D1D46] dark:text-white font-bold">
              ...
              {detail.tier === 'top' ? (
                ...
                <div className="pt-2 border-t border-dashed border-gray-100 dark:border-white/5 space-y-2">
                  ...
                  <Link
                    ...
                    className="block text-center py-2 bg-gradient-to-r from-amber-500 to-[#F0652E] text-white rounded-xl text-[9px] font-black uppercase tracking-wider hover:opacity-90 transition-opacity"
                  >
          ...
          {detail.tier === 'registered' ? (
            <div className="bg-white dark:bg-[#122238] rounded-3xl p-6 border border-gray-100 dark:border-white/5 space-y-4 relative overflow-hidden">
              <div className="absolute inset-0 bg-gray-50/70 dark:bg-[#0c1829]/70 backdrop-blur-[2px] flex flex-col items-center justify-center text-center p-4 z-10">
                <Lock className="w-8 h-8 text-amber-500 mb-2" />
                <h4 className="text-xs font-black text-[#1D1D46] dark:text-white uppercase tracking-wider">Media Gallery Locked</h4>
              ...
              <div className="grid grid-cols-2 gap-2.5 opacity-20">
                ...
                <div className="aspect-square bg-[#1D1D46]/20 dark:bg-white/5 rounded-2xl" />
              </div>
            </div>
          ) : detail.galleryImages.length > 0 ? (
            <div className="bg-white dark:bg-[#122238] rounded-3xl p-6 border border-gray-100 dark:border-white/5 space-y-4">
              <h3 className="text-sm font-black text-[#1D1D46] dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                <ImageIcon className="w-4 h-4 text-[#F0652E]" />
                Media Gallery
              </h3>
