      ...
      <article className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2 items-center">
            <span className="bg-[#1D1D46]/10 dark:bg-white/5 text-[#1D1D46] dark:text-white text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md">
              {news.category}
            </span>
            {news.isSponsored && (
              <span className="bg-[#F0652E] text-white text-[9px] font-black uppercase px-2.5 py-1 rounded-md">
                Sponsored PR
              </span>
            )}
          </div>
          <h1 className="text-2xl md:text-4xl font-extrabold text-[#1D1D46] dark:text-white leading-tight" style={{ fontFamily: 'var(--font-display)' }}>
            {news.title}
          </h1>
        ...
        <div className="relative aspect-[1.8] w-full rounded-3xl overflow-hidden bg-gray-100 dark:bg-white/5 shadow-inner">
          {news.thumbnailUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#1D1D46] to-[#0642BA] text-white/10">
        ...
        <div className="flex items-center justify-between p-4 bg-white dark:bg-[#122238] border border-gray-100 dark:border-white/5 rounded-2xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#1D1D46] to-[#0642BA] flex items-center justify-center text-white font-bold text-xs shrink-0">
            ...
            <div>
              ...
              <h4 className="text-xs font-bold text-[#1D1D46] dark:text-white">{news.company.name}</h4>
            </div>
          </div>
          <Link
            href={`/${locale}/company-news/${news.company.tier}/pages/${news.company.id}`}
            className="px-4 py-2 bg-gray-50 hover:bg-gray-100 dark:bg-white/5 dark:hover:bg-white/10 text-[#1D1D46] dark:text-white rounded-xl text-xs font-bold transition-all"
          >
            Visit Profile
          </Link>
        </div>
