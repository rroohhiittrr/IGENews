...
import LeaderOnboarding from "@/components/profile/leader/free/LeaderOnboarding";
...
export default function LeaderPlansPage() {
  ...
  // Onboarding check
  if (user.onboardingRole === "leader" && user.onboardingStatus !== "Approved" && user.onboardingStatus !== "none") {
    return <LeaderOnboarding />;
  }

  // Approved dashboard check
  if (user.onboardingRole === "leader" && user.onboardingStatus === "Approved") {
    return <LeaderDashboard />;
  }

  const currentPlan = user.leaderPlan || "none";

  const plans = [
    {
      id: "free",
      name: "FREE LEADER PROFILE",
      eligibility: "Best For: Emerging founders & executives",
      price: "₹0",
      period: "forever",
      benefits: [
        "Public leader profile",
        "Sector tagging",
    ...
    },
    {
      ...
      eligibility: "Core Value: Verified Leadership Identity",
      price: "₹39,000",
      period: "year",
      benefits: [
        ...
        "Industry visibility",
        "Sector rankings",
        "Premium SEO indexing",
      ],
      cta: "Buy Verified Plan",
    ...
    },
    {
      id: "elite",
      name: "LEADER ELITE",
      eligibility: "Core Value: Sector Influence & Executive Branding",
      price: "₹1L - ₹5L",
      period: "year",
      benefits: [
        "Premium verified positioning",
        "Homepage visibility",
        "Leadership rankings",
        "Video interviews",
        "Featured thought leadership",
        "Investor discoverability",
        "Speaking opportunities",
        "Awards jury eligibility",
        "Premium networking access",
        "AI authority recommendations",
        "Executive branding suite",
        "Sector influence positioning",
      ],
      cta: "Buy Elite Plan",
      color: "border-slate-200 dark:border-white/10 bg-white dark:bg-[#122238]",
    },
    ...
    <div className="p-5 md:p-8 lg:p-10 max-w-6xl mx-auto pb-20">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-sm text-gray-500 mt-2 max-w-lg mx-auto leading-relaxed">
          Unlock the executive Blue Tick verification, write official leadership columns, link leader profiles to company pages, and boost investor discoverability.
        </p>
      </div>
              ...
              <div>
                {isActive ? (
                  <div className="w-full py-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/50 text-emerald-700 dark:text-emerald-400 text-center font-bold text-sm rounded-2xl">
                    {plan.id === "free" ? "✓ Active Free Profile" : "✓ Your Current Plan"}
                  </div>
                ) : (
                  <button
                  ...
                  >
                    {plan.price === "₹0" ? "Activate Free Profile" : "Purchase Plan"}
                  </button>
                )}
              </div>
            </div>
